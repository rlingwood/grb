...
def chi(model, data):
    ...
    n = 0
    while n < len(data):
        sum = sum + (data[n]-model[n])**2/(3*np.sqrt(model[n]))**2
        n = n + 1
    dof = len(model) - 1
    return sum/dof
...
normalization = 10**5
initialflux = ((wl*10**6)**(beta))*normalization
...
cint = intervener(optical, xray, mw, zint)
...
while(times < 10):
    count = 0
    lvl = 100
    outputchis = np.zeros(lvl)
    outputebvs = np.zeros(lvl)
    while(count<lvl):
        flux = initialflux*attenhost(hostebv)*attenint(intebv)
        fluxwn = flux + np.random.normal(0, np.sqrt(flux))
        def model(wl, ebv):
            init = ((wl*10**6)**(beta))*normalization
            return init*np.e**(-1*chost*ebv)
        param, param_cov = curve_fit(model, wl, fluxwn)
        fittedflux = initialflux*np.e**(-1*chost*param[0])
        wlr = wl[fluxwn > 0]
        fittedfluxr = fittedflux[fluxwn > 0]
        fluxwnr = fluxwn[fluxwn > 0]
        outputchi = chi(fittedfluxr, fluxwnr)
        outputchis[count] = outputchi
        outputebvs[count] = param[0]
        count = count+1
    ...
    totalebvs = 0
    while(countt<lvl):
        totalchis = totalchis + outputchis[countt]
        totalebvs = totalebvs + outputebvs[countt]
        countt = countt + 1
    avgchi = totalchis/len(outputchis)
    avgebv = totalebvs/len(outputebvs)
    chis[times] = (round(avgchi,2))
    ebvs[times] = (round(avgebv,2))
    hostebv = hostebv + 0.1
    times = times + 1
